...
<div class="flex items-center justify-center min-h-screen">
	<div class="w-full max-w-md p-8 space-y-6 bg-paper-50 rounded-lg shadow-lg">
		...
		{#if user}
			<div class="space-y-4">
				<div>
					<h3 class="block text-sm font-medium text-muted-foreground">Nama</h3>
					<p class="mt-1 text-lg text-foreground">{user.name || 'Tidak Ada Nama'}</p>
				</div>
				<div>
					<h3 class="block text-sm font-medium text-muted-foreground">Email</h3>
					<p class="mt-1 text-lg text-foreground">{user.email}</p>
				</div>
				<div>
					<h3 class="block text-sm font-medium text-muted-foreground">Role</h3>
					<p class="mt-1 text-lg text-foreground">{userRole}</p>
				</div>
		...
		{:else}
			<p class="text-center text-muted-foreground">Memuat profil...</p>
		{/if}
	</div>
</div>