			...
			const fetchPromises = records.items.map(async (record: any) => {
				...
				return {
					...
					book: {
						...
						cover: bookRecord.cover ? pb.files.getURL(bookRecord, bookRecord.cover) : undefined,
	...
	{#if loading}
		<div class="flex items-center justify-center py-12">
			<div class="text-center">
				...
				<p class="text-muted-foreground">Memuat lemari buku...</p>
			</div>
		</div>
		...
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
			{#each lemariRecords as record}
				<div class="bg-card rounded-lg shadow-md overflow-hidden transition-all hover:shadow-lg">
					...
					<div class="p-4">
						<h3 class="font-semibold text-lg text-foreground truncate">{record.book?.judul}</h3>
						<div class="flex items-center justify-between mt-1 text-xs text-muted-foreground">
							<span>
								{record.book && record.book.penulis && record.book.penulis.length > 0 ? record.book.penulis.join(', ') : 'Penulis tidak diketahui'}
							</span>
						</div>
