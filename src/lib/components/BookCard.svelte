...
>
	...
	<div class="p-4">
		<h3 class="font-heading font-semibold text-lg text-[var(--foreground)] mb-2 line-clamp-2">
			{book.judul}
		</h3>
		...
		<div class="mb-3">
			...
			<div class="w-full bg-[var(--muted)] rounded-full h-2">
				<div
					class="bg-[var(--color-ribbon-500)] h-2 rounded-full transition-all duration-300"
			...
			{#if book.kategori.length > 0}
				<div class="flex flex-wrap gap-1">
					{#each book.kategori.slice(0, 2) as kategori}
						<span class="px-2 py-1 text-xs rounded-full bg-[var(--color-ribbon-100)] text-[var(--color-ribbon-800)]">
							{kategori}
						</span>
					{/each}
					{#if book.kategori.length > 2}
						<span class="px-2 py-1 text-xs rounded-full bg-[var(--color-paper-100)] text-[var(--color-paper-600)]">
